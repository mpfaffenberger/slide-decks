...
def data_science_vibe_coding_slide() -> RenderableType:
    ...
    data_art = pyfiglet.figlet_format("iPuppy Notebooks", font="pagga").strip()
    
    # Main title
    main_content = Text.from_markup(
        f"[bold blue]{data_art}[/bold blue]\n\n"
        "[bold yellow]iPuppy Notebooks![/bold yellow]\n\n"
        "[dim] Let's crunch some data ![/dim]",
        justify="center"
    )

    figlet = pyfiglet.figlet_format("DEMO TIME!", font="smmono12").strip()
